...
import { z } from "zod";
import { AppSetting, LEGAL_SETTINGS_KEY, defaultLegalPolicies, } from "./app-setting.model.js";
...
export function createLegalRouter(options) {
    ...
    router.put("/policies", options.authenticate, options.requireAdmin, async (req, res, next) => {
        try {
            const input = updateSchema.parse(req.body ?? {});
            const current = await loadPolicies();
            const nextDoc = await AppSetting.findOneAndUpdate({ key: LEGAL_SETTINGS_KEY }, {
                $set: {
                    privacyPolicy: input.privacyPolicy ?? current.privacyPolicy,
                    deleteAccountPolicy: input.deleteAccountPolicy ?? current.deleteAccountPolicy,
                    cancellationRefundPolicy: input.cancellationRefundPolicy ??
