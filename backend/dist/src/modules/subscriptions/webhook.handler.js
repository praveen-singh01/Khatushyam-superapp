...
import { periodDaysForPlan, unixToDate, } from "./premium.js";
...
async function findSubscription(razorpaySubscriptionId) {
    return Subscription.findOne({ razorpaySubscriptionId });
}
async function resolveUserId(input) {
    if (input.notesUserId &&
...
export function createRazorpayWebhookHandler(webhookSecret) {
    return async (req, res, next) => {
        try {
            ...
            if (!validWebhookSignature(rawBody, req.header("x-razorpay-signature"), webhookSecret)) {
            ...
            await WebhookEvent.updateOne({ provider: "razorpay", eventId }, { $set: { eventType: event || "unknown" } });
            switch (event) {
...
async function handleSubscriptionCharged(payload) {
    ...
    if (!currentEnd) {
        const days = periodDaysForPlan(sub?.plan === "weekly" ? "weekly" : "monthly");
...
async function handleSubscriptionCancelled(payload) {
    ...
    const keepUntil = (sub.currentEnd && sub.currentEnd.getTime() > now.getTime()
        ? sub.currentEnd
        : null) ??
...
async function handleSubscriptionStatusOnly(payload, status) {
    const entity = extractSubscription(payload);
    if (!entity)
        return;
    const razorpaySubscriptionId = getString(entity, "id");
