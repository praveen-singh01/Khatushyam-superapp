...
const updateUserSchema = z
    ...
    .refine((value) => value.role !== undefined ||
    value.subscriptionStatus !== undefined ||
    value.displayName !== undefined, { message: "At least one field is required" });
...
const liveStreamUpdateSchema = z
    ...
    else if (value.youtubeVideoId != null &&
...
function assetResponse(doc, cloudFrontBaseUrl) {
...
export function createAdminRouter(options) {
        ...
        try {
            ...
            const [items, total] = await Promise.all([
                User.find(filter)
                    ...
                    .skip((page - 1) * limit)
                    .limit(limit)
                    .select("firebaseUid email displayName photoUrl role subscriptionStatus razorpaySubscriptionId createdAt updatedAt")
                    .lean(),
    ...
    router.get("/content", async (req, res, next) => {
        try {
            ...
            res.json({
                items: items.map((item) => assetResponse(item, options.cloudFrontBaseUrl)),
                page,
                limit,
                total,
                totalPages: Math.ceil(total / limit) || 1,
    ...
    router.put("/story", async (req, res, next) => {
        try {
            ...
            const doc = await Story.findOneAndUpdate({ key: STORY_KEY }, {
                $set: {
                    key: STORY_KEY,
                    title: input.title,
    ...
    router.put("/live", async (req, res, next) => {
        try {
            ...
            const nextVideoId = youtubeVideoId !== undefined
                ? youtubeVideoId
            ...
            const doc = await LiveStream.findOneAndUpdate({ key: LIVE_STREAM_KEY }, {
                $set,
