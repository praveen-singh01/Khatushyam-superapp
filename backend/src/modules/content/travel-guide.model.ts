...
export interface TravelGuideItem {
  id: string;
  fromCity: LocalizedText;
  title: LocalizedText;
  steps: LocalizedText[];
}

export interface TravelGuidesDocument {
  key: string;
...
const guideSchema = new Schema<TravelGuideItem>(
  {
    ...
    steps: {
      ...
      default: [],
      validate: {
        validator: (steps: LocalizedText[]) => steps.length >= 1,
...
const travelGuidesSchema = new Schema<TravelGuidesDocument>(
  {
    key: {
      type: String,
      required: true,
    ...
    guides: {
      ...
      default: [],
      validate: {
        validator: (guides: TravelGuideItem[]) => guides.length >= 1,
...
export const TravelGuides = model<TravelGuidesDocument>(
  "TravelGuides",
  travelGuidesSchema,
);

/** Sample: how to reach Khatu — Hinglish (admin can edit anytime). */
export const DEFAULT_TRAVEL_GUIDES: TravelGuideItem[] = [
  {
