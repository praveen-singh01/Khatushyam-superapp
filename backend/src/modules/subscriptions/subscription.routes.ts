...
async function startSubscription(
  ...
  try {
    ...
    const trialExpiry = isTrial
      ? new Date(
          now.getTime() + options.trialDurationHours * 60 * 60 * 1000,
        )
      : null;
    ...
    const offerPlan: SubscriptionPlanOffer = isTrial
      ? "trial_monthly"
      : plan;
...
async function verifyPayment(
  ...
  try {
    ...
    if (
      ...
      res.json(
        entitlementFromUser(
          ...
          resolvePlan((fresh?.currentPlan as SubscriptionPlanOffer) ?? "monthly"),
          { freeMode: options.freeMode },
        ),
      );
      return;
    ...
    await User.findByIdAndUpdate(user.id, {
      ...
      razorpaySubscriptionId,
      currentPlan: resolvePlan(sub.plan as SubscriptionPlanOffer),
    });
    ...
    res.json(
      entitlementFromUser(
        ...
        resolvePlan(sub.plan as SubscriptionPlanOffer),
        { freeMode: options.freeMode },
      ),
    );
  } catch (error) {
...
export function createSubscriptionRouter(
  ...
  router.get("/status", options.authenticate, (req, res) => {
    res.json(
      entitlementFromUser(req.user!, options.planId, {
        freeMode: options.freeMode,
      }),
    );
  });
